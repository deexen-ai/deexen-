...
export default function LoginPage() {
    ...
    const { login, loginWithToken, initialize, isAuthenticated } = useAuthStore();
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            loginWithToken(token)
                ...
                .catch((err: any) => {
                    setFormError(err.message || 'OAuth login failed');
                    setIsLoading(false);
                });
        } else if (error) {
            setFormError(error);
        }
    }, [initialize, isAuthenticated, navigate, searchParams, loginWithToken]);
    ...
    const handleGithubLogin = () => {
        ...
        window.location.href = `${apiUrl}/oauth/login/github`;
    };
            ...
            <div className="w-full max-w-[360px] px-6">
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button onClick={handleGithubLogin} type="button" className="h-10 flex items-center justify-center gap-2 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-surface-hover)] hover:border-[var(--border-muted)] transition-colors">
                        ...
                        <button onClick={handleGoogleLogin} type="button" className="h-10 flex items-center justify-center gap-2 bg-[var(--bg-surface)] border border-[var(--border-default)] rounded text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-surface-hover)] hover:border-[var(--border-muted)] transition-colors">
