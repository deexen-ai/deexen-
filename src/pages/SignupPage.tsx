...
export default function SignupPage() {
    ...
    const { register, loginWithToken, initialize, isAuthenticated } = useAuthStore();
    ...
    useEffect(() => {
        ...
        // Handle OAuth token from URL (though backend redirects to /login, let's be safe)
        ...
        if (token) {
            loginWithToken(token).then(() => {
                ...
                setFormError('OAuth authentication failed');
            });
        } else if (isAuthenticated) {
            navigate('/dashboard');
        }
    }, [initialize, isAuthenticated, navigate, location.search, loginWithToken]);

    const handleOAuthLogin = (provider: 'github' | 'google') => {
        const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:8000';
        window.location.href = `${apiUrl}/oauth/login/${provider}`;
    };
                ...
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            ...
                            className="h-10 flex items-center justify-center gap-2 bg-[#141414] border border-neutral-800 rounded text-sm text-neutral-300 hover:bg-[#1a1a1a] hover:border-neutral-700 transition-colors"
                        >
